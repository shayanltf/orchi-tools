...
3. Create or update `script/build_and_run.sh`.
   ...
   - Use `references/build-run-debug/run-button-bootstrap.md` as the canonical source for the
...
4. Write `.codex/environments/environment.toml` at the project root once the script exists.
   ...
   - Use the exact action shape in `references/build-run-debug/run-button-bootstrap.md`.
...
- `references/build-run-debug/run-button-bootstrap.md`: canonical `build_and_run.sh` and `.codex/environments/environment.toml` contract.
