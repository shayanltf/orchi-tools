...
Use this skill to capture a focused, symbolicated ETTrace profile from an iOS simulator app. Pair it with `../ios/references/ios-debugger-agent.md` when the task also needs simulator build, install, launch, UI driving, logs, or screenshots.

## Core Workflow

1. Pick one focused flow and write down the expected start and stop points.
2. Build the exact simulator app being installed and profiled.
...
"$SKILL_DIR/scripts/ios-ettrace-performance/collect_ios_dsyms.sh" \
  --app "$APP" \
  --out-dir "$DSYMS" \
...
find "$RUN_DIR" -maxdepth 1 -name 'output_*.json' -newer "$CAPTURE_MARKER" -print | while IFS= read -r json; do
  ...
  {
    echo "## ${preserved##*/}"
    python3 "$SKILL_DIR/scripts/ios-ettrace-performance/analyze_flamegraph_json.py" "$preserved"
  } >> "$PRESERVED_DIR/summary.txt"
done
